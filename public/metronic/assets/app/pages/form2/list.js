...
function initTable(elSelector) {
    ...
    tableInstance = table.DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: window.App.siteUrl+'form2/list_datatable/'+window.App.uriSegment.split('/')[1],
        columns: [
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    if (full.permission.indexOf('detail') != -1) {
                        template = template + `
                            <a href="`+window.App.siteUrl+'form2/'+window.App.uriSegment.split('/')[1]+'/detail/'+full.kode['unique']+`" class="m-portlet__nav-link" title="Detail">
                                Detail
                            </a>
                        `;
                    }

                    if (full.permission.indexOf('print_rejection') != -1) {
                        template = template + `<br>
                            <a href="`+window.App.siteUrl+'form1/'+window.App.uriSegment.split('/')[1]+'/print_penolakan/'+full.kode+`" class="m-portlet__nav-link" title="Print Penolakan">
                                Print Penolakan
                            </a>
                        `;
                    }
...
function showExportModal(context) {
    ...
    for (let part = 1; part <= totalPart; part++) {
        ...
        var downloadUrl = window.App.siteUrl+'form2/'+window.App.uriSegment.split('/')[1]+`/export?part=`+part+`&dataStart=`+dataStart+`&dataEnd=`+dataEnd+`&`+dataTableFilter;
